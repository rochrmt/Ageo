...
function Securite() {
  ...
  const [form, setForm] = useState({ username: '', nom: '', password: '', role: 'user', poste: '', permissions: [] })
  ...
  const openNew = () => { setForm({ username: '', nom: '', password: '', role: 'user', poste: '', permissions: [] }); setEditing(null); setModal(true) }
  const openEdit = (u) => {
    let perms = []
    try { perms = JSON.parse(u.permissions) || [] } catch { perms = [] }
    setForm({ username: u.username, nom: u.nom, password: '', role: u.role, poste: u.poste || '', permissions: Array.isArray(perms) ? perms : [] })
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Modifier l\'utilisateur' : 'Nouvel utilisateur'} icon={UserPlus}>
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div><label className="label">Identifiant</label><input className="input" required disabled={!!editing} value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} /></div>
          </div>
