...
router.get('/session/active', async (_req, res) => {
  try {
    const session = await db.getOne(
      "SELECT TOP (1) * FROM sessions_caisse WHERE statut = 'ouverte' ORDER BY id DESC",
    )
...
router.get('/sessions', async (req, res) => {
  ...
  try {
    const rows = await db.getAll(
      `SELECT s.*,
              (SELECT ISNULL(SUM(CASE WHEN t.type = 'retrait' THEN -t.montant ELSE t.montant END), 0)
  ...
  try {
    ...
    const id = await db.insert(
      `INSERT INTO sessions_caisse (montant_ouverture, notes, statut)
       OUTPUT INSERTED.id VALUES (?, ?, 'ouverte')`,
  ...
  try {
    ...
    await db.run(
      `UPDATE sessions_caisse
          SET statut = 'fermee', date_fermeture = GETDATE(),
...
router.post('/petite-caisse/approvisionner', async (req, res) => {
  ...
  try {
    ...
    await db.transaction(async (tq) => {
      ...
      await tq.run(
        `UPDATE petite_caisse SET solde = solde + ?, updated_at = GETDATE() WHERE id = ?`,
        [mnt, pc.id],
      )
...
router.put('/petite-caisse/plafond', async (req, res) => {
  ...
  try {
    const pc = await db.getOne('SELECT * FROM petite_caisse WHERE actif = 1')
    if (!pc) return res.status(404).json({ error: 'Aucune petite caisse active' })
    await db.run('UPDATE petite_caisse SET plafond = ?, updated_at = GETDATE() WHERE id = ?', [Number(plafond) || 0, pc.id])
    res.json({ ok: true })
  } catch (err) {
