...
router.get('/synthese', async (req, res) => {
  ...
  try {
    ...
    const mensuel = await db.getAll(
      `SELECT MONTH(cmd.date_commande) AS mois, ISNULL(SUM(${LINE_TOTAL}), 0) AS ca
         FROM commandes cmd JOIN lignes_commande lc ON lc.commande_id = cmd.id
        WHERE cmd.statut = 'livree' AND YEAR(cmd.date_commande) = ?
...
router.get('/categories', async (req, res) => {
  ...
  try {
    const rows = await db.getAll(
      `SELECT ISNULL(cat.nom, 'Sans catégorie') AS categorie,
              ISNULL(SUM(${LINE_TOTAL}), 0) AS ca,
              ISNULL(SUM(lc.quantite), 0) AS quantite
