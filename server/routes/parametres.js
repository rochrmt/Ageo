...
router.get('/users', async (req, res) => {
  ...
  try {
    const rows = await db.getAll(
      `SELECT id, username, email, nom, role, poste, actif, permissions, last_login, created_at
...
router.put('/users/:id', async (req, res) => {
  if (!isAdmin(req)) return res.status(403).json({ error: 'Accès réservé aux administrateurs' })
  const { nom, email, role, poste, permissions, actif, password } = req.body || {}
  try {
    ...
    await db.run(
      `UPDATE users SET nom = ?, email = ?, role = ?, poste = ?, permissions = ?, actif = ? WHERE id = ?`,
      [nom ?? target.nom, email ?? target.email, role ?? target.role, poste !== undefined ? (poste || null) : target.poste, perms,
